...
from pages.routes.profesores import profesores_bp
from pages.routes.estudiantes import estudiantes_bp
from pages.routes.metasdeaycl import metasdea_bp
...
def verificar_login():
    ...
    rutas_libres = ["loginvic", "static", "home"]
...
def onlyboletines():
    return render_template("onlyboletinespp.html")
...
def save_areas():
    ...
    cursor.execute("""
        ...
        (nombre, scodeemp, rancode, codeprof, jefearea, alectivo, nivelesco)
...
def creaasignaturas():
    ...
    return render_template('onlycreamaterx.html', profexmateria = profexmateria, areasxmate = areasxmate, materxmate = materxmate, materfilter = materfilter, materxgrado_sel = materxgrado_sel, areasxmate_sel = areasxmate_sel, profexmateria_sel = profexmateria_sel, gradosxm_sel = gradosxm_sel)
...
def save_materia():
    ...
    #codegrad1   = request.form.get('cualgrado')       # grado
    codegrad1 = request.form.get('campo2_hidden')  # código del grado
    #rancode1    = request.form.get('rancode', '')     # si existe
    codearea1   = request.form.get('codearealistt')   # área
    #nivelmate1  = request.form.get('nivelmate', '')   # opcional
...
def asignacion(codeprof):
    ...
    cursor.execute("""
        ...
        FROM profesors, materia, colgrados
        WHERE profesors.codeprof=materia.codeprof AND materia.codegrad = colgrados.codegrad AND profesors.codeprof = %s group by materia.codegrad
    ...
    return render_template("asignacion.html", profesor=profesor, grados=grados, dirgrado = dirgrado)
...
def grado(codemate):
    ...
    cursor.execute("SELECT * FROM notastxt WHERE codemate = %s and periodo = %s and alectivo = %s", (codemate,PERIODO_ACTUAL,ALECTIVO_ACTUAL))
    ...
    #SELECT estudents.codeint,estudents.names,CONCAT(UCASE(estudents.lastn), ' ',UCASE(estudents.lastn2),', ', UCASE(names)) as nombre,estudents.lastn2, calinotas.*, estudents.alectivo,estudents.codegrad as codegrad1,estudents.scodeest  from calinotas,estudents where calinotas.codeestu = estudents.scodeest AND codemate = '" + codemate1 + "' AND calinotas.alectivo = '" + alectivo1 + "' and estudents.scodeemp = '" + scodeemp1 + "' and periodo = '" + periodo1 + "' and estudents.status = 'MATRICUL' GROUP BY estudents.scodeest order by estudents.lastn,estudents.lastn2,estudents.names
    cursor.execute("""
        SELECT 
            estudents.codeint,
            estudents.names,
    ...
    """, (codemate, ALECTIVO_ACTUAL, PERIODO_ACTUAL))

    estudiantes = cursor.fetchall()

    #SELECT estudents.codeint,estudents.names,CONCAT(UCASE(estudents.lastn), ' ',UCASE(estudents.lastn2),', ', UCASE(names)) as nombre,estudents.lastn2, calinotas.*, estudents.alectivo,estudents.codegrad as codegrad1,estudents.scodeest  from calinotas,estudents where calinotas.codeestu = estudents.scodeest AND codemate = '44' AND calinotas.alectivo = '2020' and estudents.scodeemp = '31' and periodo = 'PER3' and estudents.status = 'MATRICUL' GROUP BY estudents.scodeest order by estudents.lastn,estudents.lastn2,estudents.names

    cursor.close()
    conn.close()

    return render_template("grado.html", grado=grado, estudiantes=estudiantes, codemate=codemate, alectivo=ALECTIVO_ACTUAL, periodo=PERIODO_ACTUAL, columnas=columnas)
...
def guardar_notas():
    ...
    try:
        ...
        # 5. ACTUALIZAR ÁREA
        # ----------------------------------------------------------
        actualizar_area(periodo, alectivo, codegrad, codearea, scodeemp, conn)
...
def actualizar_area(periodo, alectivo, codegrad, codearea, scodeemp, conn):
    cursor = conn.cursor(dictionary=True)
    ...
    for fila in resultados:
        ...
        if periodo == "PER1":
            try:
                ...
                cursor.execute("""
                    UPDATE caliarea 
                    SET per1 = %s, notafinal = %s
                    WHERE codearea = %s AND codeestu = %s 
                        AND codegrad = %s 
                """, (per11, notafinal1, codearea1, codeestu1, codegrad1))
            ...
            try:
                ...
                cursor.execute("""
                        UPDATE caliarea 
                        SET per2 = %s, notafinal2 = %s
                        WHERE codearea = %s AND codeestu = %s 
                            AND codegrad = %s 
                    """, (per21, notafinal2, codearea1, codeestu1, codegrad1))
                #print("Filas afectadas:", cursor.rowcount)
            except Exception as e:
                    print("❌ Error en UPDATE:", e)
            ...
            try:
                ...
                #print(f"per31:{per31}, notafinal3:{notafinal3}")
                cursor.execute(
                    """
                    UPDATE caliarea
                    SET per3 = %s, notafinal3 = %s
                    WHERE codearea = %s AND codeestu = %s AND codegrad = %s
                    """, (per31, notafinal3, codearea1, codeestu1, codegrad1))
            ...
            try:
                ...
                cursor.execute(
                    """
                    UPDATE caliarea
                    SET per4 = %s, notafinal4 = %s
                    WHERE codearea = %s AND codeestu = %s AND codegrad = %s
                    """, (per41, notafinal4, codearea1, codeestu1, codegrad1))
...
@app.route("/guardar_onlyeditnotas/<codemate>/<prodid>", methods=["POST"])
def guardar_onlyeditnotas(codemate, prodid):
    scodeemp = SCODEEMP
    periodo = PERIODO_ACTUAL
    alectivo = ALECTIVO_ACTUAL
    ...
    return redirect(url_for("onlyeditnotas", codemate=codemate))
...
def onlylistxalumn2DIR(codegrad):
    ...
    #carga las notas por materia
    #cursor.execute("""
    #SELECT left(metsncomp.metaapr,50) as metaapr,left(metsncomp.comptlab,50) as comptlab,left(calimate.rempe4,50) as rempe4, metsncomp.periodo, materia.nombre, materia.keymate, calimate.* 
    #from calimate,materia,metsncomp 
    #where materia.codemate = calimate.codemate and metsncomp.codemate = calimate.codemate and calimate.codegrad = %s and codeestu = %s and trim(materia.scodeemp) = %s and UCASE(metsncomp.periodo) = %s 
    #order by materia.keymate
    #""", (codegrad, codeestu, scodeemp, periodo))
    #notasestuxmateria = cursor.fetchall()
...
def get_notas_estudiante(codegrad, codeestu):
    ...
    cursor.execute("""
        SELECT LEFT(metsncomp.metaapr,50) AS metaapr,
               ...
               materia.nombre,
               materia.keymate,
               obs_form.notatt AS nivelprom_text,
               obs_rem.notatt AS concepto_text,
               calimate.*
        FROM calimate
        JOIN materia   ON materia.codemate   = calimate.codemate
        JOIN metsncomp ON metsncomp.codemate = calimate.codemate
        JOIN observa AS obs_rem ON obs_rem.codeobse = 
        CASE
            WHEN UPPER(%s) = 'PER3' THEN calimate.obspe3
            WHEN UPPER(%s) = 'PER2' THEN calimate.obspe2
            WHEN UPPER(%s) = 'PER1' THEN calimate.obspe1
        END
        JOIN observa AS obs_form ON obs_form.codeobse = 
        CASE
            WHEN UPPER(%s) = 'PER3' THEN calimate.obsprom3
            WHEN UPPER(%s) = 'PER2' THEN calimate.obsprom2
            WHEN UPPER(%s) = 'PER1' THEN calimate.obsprom
            ELSE calimate.obsprom3
        END                              
        WHERE calimate.codegrad = %s
          AND calimate.codeestu = %s
          AND TRIM(materia.scodeemp) = %s
          AND UPPER(metsncomp.periodo) = %s
        ORDER BY materia.keymate
    """, (periodo,periodo,periodo,periodo,periodo,periodo,codegrad, codeestu, scodeemp, periodo))
